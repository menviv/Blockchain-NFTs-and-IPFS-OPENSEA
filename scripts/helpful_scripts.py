from brownie import network, accounts, config, VRFCoordinatorMock, MockV3Aggregator, LinkToken, Contract, web3
...
def get_contract(contract_name):
    ...
    Returns:
        brownie.network.contract.ProjectContract: The most recently deployed version of this contract (Example: MockV3Aggregator[-1])
    """
...
def deploy_mocks(decimals=DECIMALS, initial_value=INITIAL_VALUE):
    ...
    print(f"Mocks deployed...{MockV3Aggregator[-1].address}")
    link_token = LinkToken.deploy({"from": account})
    VRFCoordinatorMock.deploy(link_token, {"from": account})
    # return MockV3Aggregator[-1].address


def fund_with_linktoken(
    contract_address, account=None, link_token=None, amount=web3.toWei(1, "ether")):  # 0.1 Link
